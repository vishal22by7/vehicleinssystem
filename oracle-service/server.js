  ...
  try {
    ...
    while (retries > 0 && !connected) {
      try {
        ...
        connected = true;
        if (retries < 10) {
          console.log('✅ Connected to blockchain node (block:', blockNumber, ')');
        } else {
        ...
        if (retries > 0) {
          ...
          if ((10 - retries) % 3 === 0 || retries === 1) {
            const errorMsg = error.code === 'INVALID_ARGUMENT' 
              ? 'Node not ready yet' 
              : error.message || 'Connection error';
            console.log(`⏳ Waiting for blockchain node... (${10 - retries}/10 attempts) - ${errorMsg}`);
          }
          ...
          const errorMsg = lastError?.code === 'INVALID_ARGUMENT' 
            ? 'Node not ready or invalid response format' 
            : lastError?.code || lastError?.message || 'Connection refused';
          console.error('   Error:', errorMsg);
          // Don't return - continue with background retry
          console.log('   ⚠️  Will retry in background every 30 seconds...');
        }
      }
    }
...
initBlockchain().then(() => {
  ...
  // Retry blockchain connection in background every 30 seconds
  const retryInterval = setInterval(async () => {
    if (!contract && provider) {
      // Provider exists but contract not set - try to set contract
      try {
        if (CONTRACT_ADDRESS && PRIVATE_KEY && PRIVATE_KEY !== 'your-private-key-here') {
          wallet = new ethers.Wallet(PRIVATE_KEY, provider);
          contract = new ethers.Contract(CONTRACT_ADDRESS, contractABI, wallet);
